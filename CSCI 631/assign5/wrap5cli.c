 ...
 */
#include "wrap5cli.h" //header file
 ...
 * Change directory.
 *
 * This function will take as input a string representing the directory where the prohram needs
 * to poit at
 *
 * Usage: Chdir( char* <directory> )
 *
 * @return exit code on success
 * @error Print a message and exit the program with exit code 1.
 *        returns the real return code in case we skip the error
 */
...
int Chdir(char* directory){
    ...
    //check if the function was able to change directory
    if ( ( ret=chdir( directory ) ) == -1 ){
        //if its in error condition
        if ( !SKIP_ERROR[CHDIR]){
